...
{
  //create an instance of SingletonDatabaseCarc
  ...
  BOOST_TEST_MESSAGE( "TEST CASE 1: The number of brand ASton Martin in the database is correct" );

  BOOST_CHECK_EQUAL(database.get_nbCar("Toyota"), 1789);
  BOOST_TEST_MESSAGE( "TEST CASE 1: The number of brand Toyota in the database is correct" );  
}
...
{ 
  ...
  BOOST_TEST_MESSAGE( "TEST CASE 2: The BMW brand car doesn't exist in the database and it returns 0" );  
}
...
BOOST_AUTO_TEST_CASE( test_case3 )
{
  ...
  BOOST_TEST_MESSAGE( "TEST CASE 3: the 2 instances point to the same SingletonDatabaseCar object" );  
}
...
{ 
  ...
  if (database.get_nbCar("Mercedes") != 200)
  {
    BOOST_ERROR("TEST CASE 4: Test failed: the number of cars Mercedes is inccorect");
  }
}
